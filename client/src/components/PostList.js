...
import { compareHighestScore, comparePublished } from '../helper';
...
class PostList extends Component {
  ...
  renderPosts() {
    ...
    if (searchValue === 'votes') {
      ...
      return _.map(sortedPosts, post => <div key={post.id}>{this.renderPostsItem(post)}</div>);
    } else if (searchValue === 'published') {
      const sortedPosts = posts.sort(comparePublished);
      return _.map(sortedPosts, post => <div key={post.id}>{this.renderPostsItem(post)}</div>);
    }
    return _.map(posts, post => <div key={post.id}>{this.renderPostsItem(post)}</div>);
  }
